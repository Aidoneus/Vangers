...
char modsArray[][4][40] = 
{
	...
	{"wiring", "¯à®¢®¤ª "},
	{"zeefick", "æëä¨ª"},
	{"khox run", "­îå­ñ¬ ª®ªáã!", "­îå­ñ¬ ª®ªáã"}
};
...
MessageElement::MessageElement(const char* player_name, char* msg,int col)
{
    ...
    if (strncmp(msg, bot_tag, 5)==0) {
		...
		name = (char*)"$";
		actual_msg = (char*)"‘â àâ ç¥à¥§ 20 á¥ªã­¤";
		if (isMod(ID_MAMMOTH)) 
			actual_msg = (char*)"‘â àâ ¬ ¬®­â  ç¥à¥§ 20 á¥ªã­¤, ®å®â­¨ª®¢ ç¥à¥§ 40";
		...
		actual_col = 3;
		is_start = 1;
	} else if ((strcmp(msg, "/finish")==0||strcmp(msg, ". èâèëà")==0) && (is_start==2 || is_start==3)) {
		name = (char*)"$";
		actual_msg = (char*)"”¨­¨è";
		actual_col = 3;
		is_start = 0;
	} 
	else if ((strcmp(msg, "ï")==0||strcmp(msg, "z")==0 || strcmp(msg, "Ÿ")==0||strcmp(msg, "Z")==0) && is_start==2 && whoIsKvach==1) {
		whoIsKvach = 2;
	...
	else if ((strcmp(msg, "/rekvach")==0||strcmp(msg, ".ªã«¬äáà")==0) && is_start==2 && isMod(ID_MECHOKVACH)) {
		name = (char*)"$";
		actual_msg = (char*)"Šâ® ª¢ ç? (ï/z)";
		actual_col = 3;
		whoIsKvach=1;
		kvachTime=-1;
	}
	else if ((strcmp(msg, "/scancel")==0 || strcmp(msg, ".ëáäâáã¤")==0) && is_start==1) {
		name = (char*)"$";
		actual_msg = (char*)"‘â àâ ®â¬¥­¥­";
		actual_col = 3;
		is_start = 0;
	}
	else if ((strcmp(msg, "/rollcall")==0 || strcmp(msg, ".ªé¤¤áä¤¤")==0) && isRollcall==-1) {
		name = (char*)"$";
		actual_msg = (char*)"à¥ª«¨çª ";
		actual_col = 3;
		isRollcall = 0;
		rollcallNicknames = new char[10000]();
		rollcallNicknames[0] = ((char*)(";"))[0];
	} 
	else if ((strcmp(msg, "/rcancel")==0 || strcmp(msg, ".ªáäâáã¤")==0) && isRollcall!=-1) {
		name = (char*)"$";
		actual_msg = (char*)"à¥ª«¨çª  ®â¬¥­¥­ ";
		actual_col = 3;
		isRollcall = -1;
		rollcallNicknames = new char[10000]();
	} 
